...
function makeKeyboardPractice(stage) {
    ...
    new Label({color:purple, text:"Keyboard", size:45,variant:true}).pos(0,70,CENTER,TOP,page);

    //const label = new Label("hello").center().mov(0,-100)
    ...
    // })//.show();
    //console.log(key)
    const label = new Label("").pos(0,0,CENTER,CENTER,page)//.center().mov(0,-100)

    let key = new Keyboard({
        labels:label, 
        layout:"ARABIC",
    }).pos(0,70,CENTER,BOTTOM,page)
   // STYLE = {spacing:20, corner:5, base:"none"};
   //  const tabs = new Tabs(800,50,["ARABIC","HEBREW","TURKISH","AZERTY","QWERTY"])
   //      .pos(0,100,CENTER)
   //      .change(() => {
   //          zog(tabs.text)
   //          key.dispose();
   ...
   //  }).show();
    ...
    //makeHeader("ZIM Cat - Keyboard Layouts", "Keyboard", true);
